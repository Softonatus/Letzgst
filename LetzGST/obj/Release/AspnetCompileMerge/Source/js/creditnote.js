...
function getLastCreditId() {
    var req = { "userId": "" };
    var getLastBillIdUrl = generateHandlerUrl.getUrl("../HandlerFiles/Manage/GetLastInsertedCreditNoteId.ashx");
    var params = $.extend({}, doAjax_params_default);
    params['url'] = getLastBillIdUrl;
    params['data'] = req;
    params['successCallbackFunction'] = responseDetails.retriveLastCreditId;
    doAjax(params);
}
...
function getCreditNoteList() {
    var req = { "userId": "" };
    var getDebitListurl = generateHandlerUrl.getUrl("../HandlerFiles/Manage/CreditNoteDetails.ashx?opertype=get");
...
var responseDetails = {
    getCustomerdetails: function (data) {
        ...
        if (RespCode == 0) {
            if (data.resCustomer != null) {
                ...
                $("#creditnoteto").append(html);
                $("#creditnoteto").trigger("chosen:updated");
    ...
    retriveLastCreditId: function (data) {
        if (parseInt(data.ResponseCode) == 0) {
    ...
    insertCreditNote: function (data) {
        if (parseInt(data.ResponseCode) == 0) {
    ...
    getCreditNoteDetails: function (data) {
        ...
        if (data.resCreditGetList != null) {
            var myTemplate = $.templates("#tblCreditNoteDetails");
            var html = myTemplate.render(data.resCreditGetList);
            $("#creditDetails").html(html);
            addSerialNumber("creditDetails");
            $('#creditlisttbl').DataTable({
                responsive: true,
                columnDefs: [
                    {
                        width: 10,
    ...
    deleteCreditNotes: function (data) {
        if (parseInt(data.ResponseCode) == 0) {
            AlertSuc.alertsuccess("success", data.ResponseMessage);
    ...
    getCreditDetailsbyId: function (data) {
        if (data.resCreditGetList != null) {
            $('#creditnoteto').val(data.resCreditGetList[0].CustomerId);
            $("#billno").val(data.resCreditGetList[0].CreditNo).attr("disabled", "disabled");
            $("#podate").val(convertDate(data.resCreditGetList[0].CreditDate.split("T")[0]));            
            $("#invrefno").val(data.resCreditGetList[0].InvoiceRefNo);
            $("#refno").val(data.resCreditGetList[0].RefNo);
            $("#shipaddress1").val(data.resCreditGetList[0].ShippingAddress1);
            $("#shipaddress2").val(data.resCreditGetList[0].ShippingAddress2);
            $("#shipcity").val(data.resCreditGetList[0].City);
            $("#pincode").val(data.resCreditGetList[0].PinCode);
            $("#shippingState").val(data.resCreditGetList[0].State);
            $("#disount").val(data.resCreditGetList[0].Discount);

            $("#freight").val(data.resCreditGetList[0].Frieght);
            $("#labourCharge").val(data.resCreditGetList[0].LabourCharge);
            $("#InsuranceAmt").val(data.resCreditGetList[0].InsuranceAmount);
            $("#otherCharges").val(data.resCreditGetList[0].OtherCharge);
            if (data.resCreditGetList[0].Cess) {
                $("#cess").val(data.resCreditGetList[0].Cess);
            }
            $("#issueReason").val(data.resCreditGetList[0].ReasonOfIssuingDocument);
            $("#TandC").val(data.resCreditGetList[0].TermCondition);
            $("#INVPayDet").val(data.resCreditGetList[0].InvoicePaymentDetail);
            if (data.resCreditGetList[0].ResCreditItemList != null) {
                if (data.resCreditGetList[0].ResCreditItemList.length > 0) {
                    $("#tblItemDetails").find("tr:gt(0)").remove();
                    if (data.resCreditGetList[0].ResCreditItemList.length > 1) {
                    ...
                    for (var b = 1; b <= (data.resCreditGetList[0].ResCreditItemList.length) ; b++) {
                        if (b > 1) {
                            ...
                            if (b < data.resCreditGetList[0].ResCreditItemList.length) {
                    ...
                    var json = data.resCreditGetList[0].ResCreditItemList;
                    for (var key in json) {
                        if (json.hasOwnProperty(key)) {
                            ...
                            $("#P_Discount_" + a).text(data.resCreditGetList[0].Discount);
            ...
            $("#TA_notes").val(data.resCreditGetList[0].Note);

            //for PDF code
            $('#P_invoiceto').text(data.resCreditGetList[0].CustomerId);
            $("#P_billno").text(data.resCreditGetList[0].InvoiceNo).attr("disabled", "disabled");
            $("#P_invNO").text(data.resCreditGetList[0].InvoiceNo);
            $("#P_podate").text(convertDate(data.resCreditGetList[0].CreditDate.split("T")[0]));
            //$("#P_duedate").text(convertDate(data.resCreditGetList[0].DueDate.split("T")[0]));
            $("#P_refno").text(data.resCreditGetList[0].RefNo);
            $("#P_shipadd1").text(data.resCreditGetList[0].ShippingAddress1);
            $("#P_shipadd2").text(data.resCreditGetList[0].ShippingAddress2);
            $("#P_city").text(data.resCreditGetList[0].City);
            $("#P_pincode").text(data.resCreditGetList[0].PinCode);
            $("#P_billingstate").text(data.resCreditGetList[0].State);
            $("#P_freight").text(data.resCreditGetList[0].Frieght);
            $("#P_labourCharge").text(data.resCreditGetList[0].LabourCharge);
            $("#P_InsuranceAmt").text(data.resCreditGetList[0].InsuranceAmount);
            $("#P_otherCharges").text(data.resCreditGetList[0].OtherCharge);
            $("#P_disount").text(data.resCreditGetList[0].Discount);
            $("#P_TandC").text(data.resCreditGetList[0].TermCondition);
            $("#P_INVPayDet").text(data.resCreditGetList[0].InvoicePaymentDetail);

            $(".chosen-select-deselect").trigger("chosen:updated");

            $("#creditnotetable").hide(800);
            $("#creditnote").show(800);
    ...
    P_editCustDetails: function (data) {
        if (parseInt(data.ResponseCode) == 0) {
            var EmailBody = "HEREWITH ENCLOSED THE CREDIT NOTE RAISED AGAINST INVOICE REF NO-" + $("#invrefno").val() + "  FOR YOUR REVIEW.";
            $("#EmailBody").val(EmailBody)
            $("#EmailSubject").val("Credit Note against Invoice Ref No-" + $("#invrefno").val());
            var sez = false;
...
$(document).ready(function () {
    ...
    $('#backtocredit').click(function () {
        AlertSuc.clear_form_elements("frmCreditNote");
        $("#frmCreditNote").find('select').each(function () {
            $(this).val("");
        });
        $(".chosen-select-deselect").trigger("chosen:updated");
        $("#creditnote").hide(800);
        $("#creditnotetable").show(800);
    ...
    $(document).on("click", ".delete", function () {
        ...
        $("#hdnPurchaseId").val(EditPOId);swal({
            title: "Are you sure?",
        ...
        function () {
            var getdeletePOurl = generateHandlerUrl.getUrl("../HandlerFiles/Manage/CreditNoteDetails.ashx?opertype=delete");
            var params = $.extend({}, doAjax_params_default);
            params['url'] = getdeletePOurl;
            params['data'] = JSON.stringify(reqId);
            params['requestType'] = "POST";
            params['successCallbackFunction'] = responseDetails.deleteCreditNotes;
            doAjax(params);
        });
    });
    ...
    $(document).on("click", ".edit", function () {
        ...
        var getPObyIdurl = generateHandlerUrl.getUrl("../HandlerFiles/Manage/CreditNoteDetails.ashx?opertype=getcreditNoteDetails");
        var params = $.extend({}, doAjax_params_default);
        params['url'] = getPObyIdurl;
        params['data'] = JSON.stringify(reqId);
        params['requestType'] = "POST";
        params['successCallbackFunction'] = responseDetails.getCreditDetailsbyId;
    ...
    $("#frmCreditNote").validate({
        rules: {
            ...
            creditnoteto: { valueNotEquals: "" },
            invrefno: { valueNotEquals: "" },
        },
        messages: {
            creditnoteto: { valueNotEquals: "Please select a Expense Type" },
            invrefno: { valueNotEquals: "Please select a Invoice reference" }
        },
    ...
    $("#btn_saveCredit").click(function () {
        if($("#frmCreditNote").valid()){
            ...
            BillObject = {
                "CreditId": $("#hdnPurchaseId").val(),
                "CustomerId": $("#creditnoteto").val(),
                "CreditNo": $("#billno").val(),
                "CreditDate": $("#podate").val(),
                ...
                "ReasonOfIssuingDocument": $("#issueReason").val(),
                //"IsTaxInclusive": $('#Inclusive').is(':checked') ? true : false,
                "Advance": null,
                "ReqCreditItemList": PurchaseItemList
            }
            var savePOurl = generateHandlerUrl.getUrl("../HandlerFiles/Manage/CreditNoteDetails.ashx?opertype=insert");
            if ($("#hdnPurchaseId").val() != "") {
                savePOurl = generateHandlerUrl.getUrl("../HandlerFiles/Manage/CreditNoteDetails.ashx?opertype=update");
            }
            var params = $.extend({}, doAjax_params_default);
            params['url'] = savePOurl;
            params['data'] = JSON.stringify(BillObject);
            params['requestType'] = "POST";
            params['successCallbackFunction'] = responseDetails.insertCreditNote;
            doAjax(params);
        }
    });
    ...
    $(document).on('keyup change', '#disount, #creditnoteto, #shippingState, #freight, #labourCharge, #InsuranceAmt, #otherCharges, #cess', function () {
        $("#tblItemDetails tr").each(function () {
    ...
    $("#btnSendQuote").click(function () {
        ...
        $("#hdnCustomerId").val($("#creditnoteto").val());
        CustRequest = {
            "Id": $("#creditnoteto").val(),
        }
